...
public abstract class VMFValue
{
    ...
    static VMFValue()
    {
        ...
        foreach (var type in Assembly.GetExecutingAssembly().GetTypes())
        {
            ...
            {
                var patternp = type.GetField("Pattern", BindingFlags.Public | BindingFlags.Static);
                var orderp = type.GetField("Order", BindingFlags.Public | BindingFlags.Static);

                var pattern = (string)patternp.GetValue(null);
                var order = (int)orderp.GetValue(null);
    ...
    {
        foreach (var type in stTypes)
        {
            if (type.Item2.IsMatch(str))
            {
                var val = (VMFValue)type.Item1.Invoke(Array.Empty<object>());
                try
                {
                    val.String = str;
                }
...
public class VMFTextureInfoValue : VMFValue
{
    ...
    public override string String
    {
        ...
        set
        {
            ...
            var split0 = value.IndexOf(' ');
            var split1 = value.IndexOf(' ', split0 + 1);
            var split2 = value.IndexOf(' ', split1 + 1);
            var split3 = value.IndexOf(' ', split2 + 1);
...
public class VMFVector3ArrayValue : VMFValue
{
    ...
    public override VMFValue Clone()
    {
        var arr = new VMFVector3ArrayValue();
